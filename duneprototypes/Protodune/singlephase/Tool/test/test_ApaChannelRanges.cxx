...
int test_ApaChannelRanges(bool useExistingFcl =false, int show =1) {
  const string myname = "test_ApaChannelRanges: ";
#ifdef NDEBUG
  ...
  string fclfile = "test_ApaChannelRanges.fcl";
  if ( ! useExistingFcl ) {
    cout << myname << "Creating top-level FCL." << endl;
    ofstream fout(fclfile.c_str());
    fout << "tools: {" << endl;
    fout << "  mytool: {" << endl;
    fout << "    tool_type: ApaChannelRanges" << endl;
    fout << "    LogLevel: 2" << endl;
    fout << "    ApaNumbers: [3, 5, 2, 6, 1, 4]" << endl;
    fout << "    ApaLocationNames: [\"R1\", \"L1\", \"R2\", \"L2\", \"R3\", \"L3\"]" << endl;
...
int main(int argc, char* argv[]) {
  ...
  return test_ApaChannelRanges(useExistingFcl, show);
}
